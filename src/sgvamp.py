...
class VAMP:
    ...
    def infer(self,R,r,M,N,iterations,cg_maxit=500,learn_gamw=True, lmmse_damp=True):
        ...
        for it in range(iterations):
            ...
            xhat1 = np.array([self.denoiser_meta(r1s[:,i], gam1s) for i in range(M)]).reshape((M,1))
            ...
            alpha1 = np.mean(np.array([self.der_denoiser_meta(r1s[:,i], gam1s) for i in range(M)]))
